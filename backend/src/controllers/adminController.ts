...
export const createTherapist = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    // ✅ Validaciones SIN firstName ni lastName
    ...
    // Verificar si el nationalId ya existe
    ...
    // ✅ Crear usuario SIN firstName y lastName (usar email como nombre temporal)
    const user = new User({
      email,
      password,
      firstName: 'Therapist',   // ✅ Valor por defecto
      lastName: nationalId,     // ✅ Usar nationalId como apellido temporal
      role: 'therapist'
    });
    await user.save();
...
export const getAllTherapists = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
...
export const getTherapistById = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
...
export const updateTherapist = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    // Actualizar campos
    if (nationalId) therapist.nationalId = nationalId;
