import Therapist, { CATEGORIES, SPECIALTIES_BY_CATEGORY, ALL_SPECIALTIES, SESSION_TYPES, FOCUS_AREAS } from '../models/Therapist';
...
export const getAllTherapistsPublic = async (req, res) => {
    try {
        const { specialty, category, location, locationType, language, sessionType, focusArea, slidingScale, acceptsInsurance, freeConsultation, minExperience, licensed } = req.query;
...
export const getTherapistByIdPublic = async (req, res) => {
    try {
        const { id } = req.params;
...
export const createTherapist = async (req, res) => {
    try {
        const { email, password, firstName, lastName, nationalId, specialty, category, additionalSpecialties, location, locationType, city, state, country, language, dateOfBirth, bio, credentials, yearsExperience, certifications, licensedCertified, sessionTypes, focusAreas, hourlyRate, slidingScale, acceptsInsurance, freeConsultation, phone, website } = req.body;
...
export const getAllTherapistsAdmin = async (req, res) => {
    try {
        if (req.user?.role !== 'admin') {
            res.status(403).json({
                success: false,
...
export const updateTherapist = async (req, res) => {
    try {
        ...
        const updatedTherapist = await Therapist.findByIdAndUpdate(id, updateData, { new: true, runValidators: true }).populate('userId', 'firstName lastName email role');
        if (!updatedTherapist) {
            res.status(404).json({
                success: false,
                message: 'Terapeuta no encontrado'
...
export const reactivateTherapist = async (req, res) => {
    try {
        const { id } = req.params;
        if (req.user?.role !== 'admin') {
            res.status(403).json({
...
export const toggleTherapistStatus = async (req, res) => {
    try {
        const { id } = req.params;
        if (req.user?.role !== 'admin') {
            res.status(403).json({
...
export const hardDeleteTherapist = async (req, res) => {
    try {
        const { id } = req.params;
        if (req.user?.role !== 'admin') {
            res.status(403).json({
        ...
        allowedFields.forEach(field => {
            if (updateData[field] !== undefined) {
                if (field === 'language' && !Array.isArray(updateData[field])) {
                    therapist[field] = [updateData[field]];
                }
                else if (field === 'dateOfBirth') {
                    therapist[field] = new Date(updateData[field]);
                }
                else {
                    therapist[field] = updateData[field];
                }
